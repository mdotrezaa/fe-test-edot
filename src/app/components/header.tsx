  ...
  useEffect(() => {
    const timeout = setTimeout(
      () => {
        if (isDeleting) {
          if (charIndex > 1) {
            // Stop deleting when only "O" is left
  ...
  return (
    <div className="h-[calc(100vh-100px)] container mx-auto flex justify-center items-center">
      <div className="grid grid-cols-1 md:grid-cols-2 md:gap-4 w-full max-w-4xl">
        <div className="mb-4">
          <h1 className="text-4xl md:text-6xl text-center md:text-start font-bold">
            Welcome to <br /> <span className="text-primary">{text}</span>
          </h1>
          <p className="text-sm md:text-md text-gray-500 mt-3 text-center md:text-start">
          ...
          <div className="flex justify-center md:justify-start mt-3">
            <button
              ...
              className="bg-primary hover:bg-orange-500 font-semibold text-white px-4 py-2 text-sm rounded-lg"
            >
              Go to Product
            </button>
          </div>
